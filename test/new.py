...
def generate_sequences_cython(chain: List[List[Any]], last_hidden_state: int, probs: List[List[float]],
                              start_idx: int, tokens: List[int], token_pos: int):
    if start_idx >= last_hidden_state or token_pos >= len(tokens):
        if len(chain) > 2:
            return chain
...
def generate_sequences_cython_cuda(chain: List[List[Any]], last_hidden_state: int, probs: torch.Tensor,
                              start_idx: int, tokens: List[int], token_pos: int):
    if start_idx >= last_hidden_state or token_pos >= len(tokens):
        if len(chain) > 2:
            return chain
    ...
    for idx in range(start_idx, last_hidden_state):
        ...
        if prob >= 0.05:
            ...
            if len(chain) == 1:
                ...
                chain.append(current_chain)
            generate_sequences_cython_cuda(chain, last_hidden_state, probs,
                                      idx+1, tokens, token_pos+1)
        else:
            if len(chain) > 2:
                return chain
...
def main(gpt_response) -> tuple[list[str], Union[int, list[int]], list[str], list[Any], str]:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for token_pos, (token, token_id, source) in enumerate(zip(gpt_tokens, gpt_token_ids, sources)):
        # print(token_pos,"/",len(gpt_tokens))
        ...
        for batch in range(0, len(wiki_token_ids), 512):
            ...
            # result_tensor_per_token.append(probs).to(device)
            result_tensor_per_token = torch.cat((result_tensor_per_token, probs), dim=0).to(device)
        empty_chain = Chain([], [], source)
        # # source цеплять тут и выносить в лист
        generate_sequences(empty_chain, len(result_tensor_per_token), result_tensor_per_token, 0, gpt_token_ids, token_pos)
        # print("len1:", len(result_chains))
        # print("11111",result_chains)
        # empty_chain = [[]]
        # # chains = work.generate_sequences_cython(empty_chain, len(result_tensor_per_token), ff, 0,
        #       #                             gpt_token_ids, token_pos)
        # chains = generate_sequences_cython(empty_chain, len(result_tensor_per_token), result_tensor_per_token.tolist(), 0,
        #                                    gpt_token_ids, token_pos)

        # if chains is not None:
        #     result_chains_two.append(chains)
        # print("result_chains_two",result_chains_two)
        # print(len(result_chains_two))
        if token_pos == 2:
            break
        # print("source:", source)  !!!!!
    ...
    return gpt_tokens, gpt_token_ids, sources, filtered_chains, result_html
...
if __name__ == "__main__":
    gpt_tokens, gpt_token_ids, sources, result_chaines_2, html = main(
    ...
    # print("res:::", result_chains.__str__())
